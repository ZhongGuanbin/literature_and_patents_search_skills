...
- 正常 Skill 下载显式禁用 `Sci-Hub`；其余允许渠道保持 registry 顺序。
...
Skill 的授权下载总是显式禁用第一项，但不改变后续顺序。部分名义渠道实际通过公开 API 或 repository resolver 找到文件；报告精确 executed resolver，不把 alias 记为原生 PDF 成功。
